	...
	<?php
		...
		function backfill_attachment_urls() {
			...
			foreach ( $this->url_remap as $from_url => $to_url ) {
				...
				$replace_meta_query = $wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key IN ( 'enclosure' )", $from_url, $to_url);

				$result = $wpdb->query( $replace_meta_query );

				// $replace_meta_query = $wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key='_elementor_data'", str_replace('/','\\\\\\/', $from_url ), str_replace('/','\\\\\\/', $to_url ));
				$replace_meta_query = "UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, '" .  str_replace('/','\\\\\\/', $from_url ) . "',  '" .  str_replace('/','\\\\\\/', $to_url ) . "') WHERE meta_key='_elementor_data'";
