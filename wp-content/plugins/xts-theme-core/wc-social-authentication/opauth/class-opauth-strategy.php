...
class OpauthStrategy {
	...
	private function shipToCallback( $data, $transport = null ) {
		...
		switch ( $transport ) {
			case 'get':
				$this->redirect( $this->env['callback_url'] . '?' . http_build_query( array( 'opauth' => xts_compress( serialize( $data ) ) ), '', '&' ) );
				break;
			case 'post':
				$this->clientPost( $this->env['callback_url'], array( 'opauth' => xts_compress( serialize( $data ) ) ) );
				break;
			case 'session':
			default:
				if ( ! session_id() ) {
